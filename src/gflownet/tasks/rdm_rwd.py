...
class RdmRwdTask(GFNTask):
    ...
    def compute_reward_from_graph(self, graphs: List[Data]) -> Tensor:
        ...
        # replace the reward by some random numbers
        low = 1e-4
        high = 100
        preds = low + (high - low) * torch.rand(len(preds))
        preds[preds.isnan()] = 0
...
class LittleSEHDataset(Dataset):
    ...
    def setup(self, task: RdmRwdTask, ctx: FragMolBuildingEnvContext) -> None:
